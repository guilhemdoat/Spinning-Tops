...
//Opérateurs--------------------------------------------
  ...
  Matrice33 M_oppose() const; //oppose additivement la matrice

    Matrice33 operator-(Matrice33 autre) const;

  Matrice33 transp(); //transposee

  double det() const; //determinant

  Matrice33  inv() const; //inverse

  const Matrice33 operator*(Matrice33 const& autre) const; //produit matriciel

  const Vecteur operator*(Vecteur const& autre) const; //multiplication a droite par un vecteur
