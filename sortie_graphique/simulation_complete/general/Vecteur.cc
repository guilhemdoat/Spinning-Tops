...
using namespace std;
  ...
  //utile tres souvent
bool compare_(double x, double y, double precision) {return abs(x-y) < precision;}
...
//constructeurs
  ...
  Vecteur::Vecteur(Vecteur const& autre) : v(autre.get_vect()) {}
...
//======================== autre methode ===========================
//utilisee dans l'addition (operator+) et le produit scalaire (operator*)
...
bool Vecteur::operator==(Vecteur const& vc) const {
...
bool Vecteur::operator!=(Vecteur const& vc) const  {
  ...
  const Vecteur Vecteur::operator+(Vecteur const& autre) const{ //voir question P1.2 pour le cas ou la dimension
                                                                //des deux vecteurs est differente
...
double Vecteur::operator*(Vecteur autre) const{ // produit scalaire
 double result(0.0);
  size_t taille_max;
  size_t taille_min;
  vector<double> grand, petit;
...
void Vecteur::operator*=(double lambda){
  *this = (*this)*lambda;
}
