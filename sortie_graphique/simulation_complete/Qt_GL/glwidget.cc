#include <QKeyEvent>
#include <QTimerEvent>
#include <QMatrix4x4>
#include <fstream>
#include <vector>
#include <string>
#include "glwidget.h"
#include "Systeme.h"
#include "ConeSimple.h"
#include "Vecteur.h"
#include "IntEulCromer.h"

// ======================================================================
void GLWidget::initializeGL() //méthode d'initialisation non modifiée
{
  vue.init();
  timerId = startTimer(20);

}

// ======================================================================
void GLWidget::resizeGL(int width, int height) //méthode de resize non modifiée
{
  /* On commance par dire sur quelle partie de la 
   * fenêtre OpenGL doit dessiner.
   * Ici on lui demande de dessiner sur toute la fenêtre.
   */
  glViewport(0, 0, width, height);

  /* Puis on modifie la matrice de projection du shader.
   * Pour ce faire on crée une matrice identité (constructeur 
   * par défaut), on la multiplie par la droite par une matrice
   * de perspective.
   * Plus de détail sur cette matrice
   *     http://www.songho.ca/opengl/gl_projectionmatrix.html
   * Puis on upload la matrice sur le shader à l'aide de la
   * méthode de la classe VueOpenGL
   */
  QMatrix4x4 matrice;
  matrice.perspective(70.0, qreal(width) / qreal(height ? height : 1.0), 1e-3, 1e5);
  vue.setProjection(matrice);
}

// ======================================================================
void GLWidget::paintGL() //méthode d'affichage non modifiée
{
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  c.dessine();
}


// ======================================================================
void GLWidget::keyPressEvent(QKeyEvent* event)// méthode de gestion des évenements clavier dans laquelle on a rajouter toues les modifications de la vue implémentée dans vueopengl
{
  constexpr double petit_angle(5.0); // en degrés
  constexpr double petit_pas(1.0);

  switch (event->key()) {

  case Qt::Key_Left:
    vue.rotate(petit_angle, 0.0, -1.0, 0.0);
    break;

  case Qt::Key_Right:
    vue.rotate(petit_angle, 0.0, +1.0, 0.0);
    break;

  case Qt::Key_Up:
    vue.rotate(petit_angle, -1.0, 0.0, 0.0);
    break;

  case Qt::Key_Down:
    vue.rotate(petit_angle, +1.0, 0.0, 0.0);
    break;

  case Qt::Key_PageUp:
  case Qt::Key_W:
    vue.translate(0.0, 0.0,  petit_pas);
    break;

  case Qt::Key_PageDown:
  case Qt::Key_S:
    vue.translate(0.0, 0.0, -petit_pas);
    break;

  case Qt::Key_A:
    vue.translate( petit_pas, 0.0, 0.0);
    break;

  case Qt::Key_D:
    vue.translate(-petit_pas, 0.0, 0.0);
    break;

  case Qt::Key_R:
    vue.translate(0.0, -petit_pas, 0.0);
    break;

  case Qt::Key_F:
    vue.translate(0.0,  petit_pas, 0.0);
    break;

  case Qt::Key_Q:
    vue.rotate(petit_angle, 0.0, 0.0, -1.0);
    break;

  case Qt::Key_E:
    vue.rotate(petit_angle, 0.0, 0.0, +1.0);
    break;

  case Qt::Key_C:
    vue.initializePosition();
    break;

  case Qt::Key_V:
      vue.vuedecoteCentrale();
      break;

  case Qt::Key_N:
      vue.vuedecoteDroite();
      break;

  case Qt::Key_B:
      vue.vuededessusDroite();
      break;

  case Qt::Key_Z:
      vue.vuededessusGauche();
      break;

  case Qt::Key_X:
      vue.vuedecoteGauche();
      break;

  case Qt::Key_G:
      vue.vuededessusarriere();
      break;

  case Qt::Key_H:
      vue.vuedecotearriere();
      break;

  case Qt::Key_Space:
	pause();
	break;
  };

  update(); // redessine
}

// ======================================================================
void GLWidget::timerEvent(QTimerEvent* event) // méthode d'évolutution
{
  Q_UNUSED(event);

  double dt = 0.01;
  double t_debut(0);


  for(int i(0) ; i < 4 ; ++i) { // on a ici une boucle sur le nombre de toupie et d'intégrateurs
      try {
          c.evolue(dt, t_debut,i,i); // on fait évoluer chaque couple toupie/intégrateur
      } catch (std::string const& error) { // on gère l'erreur si besoin est
          std::cerr << error << std::endl;
      }

  }

  update();


}

// ======================================================================
void GLWidget::pause() // méthode de pause non modifiée
{
  if (timerId == 0) {
	// dans ce cas le timer ne tourne pas alors on le lance
	timerId = startTimer(20);
        chronometre.restart();
  } else {
	// le timer tourne alors on l'arrête
	killTimer(timerId);
	timerId = 0;
  }
}

//============================================================================

